...
@Component({
  tag: 'admgold24k-modal',
  styleUrl: 'admgold24k-modal.css',
  shadow: true,
  assetsDirs: ['assets'],
})
export class Admgold24kModal {
  @Prop({
    mutable: true,
    reflect: true,
  })
        ...
        <div class="modal">
          ...
          <div class="footer">
            {this._buttons.map((button, index) => (
              <admgold24k-button onClick={index === 0 ? this.handleAction : this.handleCancel} text={button.text} appearance={index === 0 && this.appearance} />
            ))}
          </div>
        </div>
      </div>
